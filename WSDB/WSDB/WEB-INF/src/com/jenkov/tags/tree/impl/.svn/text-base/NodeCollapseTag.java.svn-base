...
public class NodeCollapseTag extends NodeBaseTag{
    ...
    public boolean hasCollapseHandle(ITreeIteratorElement element){
       return (element.isExpanded()) && (element.getNode().hasChildren());
    }

    public int doStartTag() throws JspException{
        ITreeIteratorElement element = getElement();
        if(hasCollapseHandle(element)){
            write("<a href=\"");
            write(getTarget());
            write("?");
            write(getCollapseParam());
            write("=");
